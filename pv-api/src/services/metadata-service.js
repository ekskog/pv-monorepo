  ...
  async extractEssentialMetadata(buffer, filename) {
    ...
    try {
      ...
      if (this.isHeic(buffer)) {
        ...
        if (date) {
          try { metadata.timestamp = new Date(date).toISOString(); } catch (_) {}
        }
        ...
        metadata.dimensions.width        = raw.ImageWidth   || raw.ExifImageWidth  || "not found";
        metadata.dimensions.height       = raw.ImageHeight  || raw.ExifImageHeight || "not found";
        ...
        raw = await exifr.parse(buffer, {
          ...
          pick: [
            ...
            "ISO", "ISOSpeedRatings", "FNumber", "ApertureValue", "ExposureTime", "ShutterSpeedValue",
        ...
        for (const field of ["DateTimeOriginal", "CreateDate", "DateTime", "DateTimeDigitized"]) {
          if (raw[field]) {
            try { metadata.timestamp = new Date(raw[field]).toISOString(); break; } catch (_) {}
          }
        }

        let lat = raw.latitude  ?? (Array.isArray(raw.GPSLatitude)  ? this.dmsToDecimal(...raw.GPSLatitude,  raw.GPSLatitudeRef  || "N") : undefined);
        let lng = raw.longitude ?? (Array.isArray(raw.GPSLongitude) ? this.dmsToDecimal(...raw.GPSLongitude, raw.GPSLongitudeRef || "E") : undefined);
        ...
        metadata.settings.aperture     = raw.FNumber       || raw.ApertureValue     || "not found";
        metadata.settings.shutterSpeed = raw.ExposureTime  || raw.ShutterSpeedValue || "not found";
        metadata.settings.focalLength  = raw.FocalLength   || "not found";
        metadata.settings.flash        = raw.Flash         || "not found";
        metadata.settings.whiteBalance = raw.WhiteBalance  || "not found";
  ...
  async getAddressFromCoordinates(coordinates, filename) {
    ...
    try {
      ...
      if (data.features?.length > 0) {
        const address = data.features[0].place_name || data.features[0].text || "Address not found";
  ...
  async updateFolderMetadata(bucketName, objectName, metadata) {
    ...
    try {
      ...
      try {
        ...
        folderData = { folderName, media: [], lastUpdated: new Date().toISOString() };
      }
      ...
      await this.minioClient.putObject(bucketName, jsonFileName, Buffer.from(JSON.stringify(folderData, null, 2)));
      return true;
    } catch (error) {
